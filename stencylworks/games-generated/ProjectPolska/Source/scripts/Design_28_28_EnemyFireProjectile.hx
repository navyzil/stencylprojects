...
class Design_28_28_EnemyFireProjectile extends ActorScript
{          	
...
propertyChanged("_Firing", _Firing);
            ...
            getLastCreatedActor().setVelocity(Utils.DEG * (Math.atan2(((actor.getY() + 20) - actor.getY()), ((actor.getX() + 12) - actor.getXCenter()))), _FiringSpeed);
            runLater(1000 * _Delay, function(timeTask:TimedTask):Void {
                        _Firing = false;
propertyChanged("_Firing", _Firing);
}, actor);
